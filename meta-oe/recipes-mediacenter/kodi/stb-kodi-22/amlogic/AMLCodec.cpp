...
#if defined(STB_AMLOGIC_LEGACY_AMSTREAM)
// DreamOne/Two do not feed compressed frames through libamcodec. Their E2
// dreamvideosink uses the DVB video device and passes one access unit (plus
// optional headers) with VIDEO_SET_FRAME. Keep these structures ABI-identical
// to the Dream 4.9 driver; the ioctl sizes are part of the command numbers.
struct DreamVideoFrame
{
  uint64_t pts;
  ssize_t bytes[8];
...
static_assert(sizeof(DreamVideoFrame) == 0xa8, "Dream VIDEO_SET_FRAME ABI mismatch");
static_assert(sizeof(DreamVideoDecSysInfo) == 0x30,
              "Dream VIDEO_SET_DEC_SYSINFO ABI mismatch");

#define DREAM_VIDEO_SET_FRAME _IOWR('o', 64, DreamVideoFrame)
#define DREAM_VIDEO_SET_DEC_SYSINFO _IOWR('o', 65, DreamVideoDecSysInfo)
#endif
...
namespace
{
constexpr int DREAM_STREAMTYPE_FROM_SYSINFO = -2;

int DreamStreamType(vformat_t format)
{
  // Values verified against the proprietary aarch64 dreamvideosink.
  switch (format)
  {
    case VFORMAT_MPEG12:
      return 0;
    ...
    case VFORMAT_AVS:
    case VFORMAT_AVS2:
      // The Dream driver selects AVS/AVS2 from VIDEO_SET_DEC_SYSINFO. The
      // proprietary sink deliberately does not issue VIDEO_SET_STREAMTYPE.
      return DREAM_STREAMTYPE_FROM_SYSINFO;
    case VFORMAT_HEVC:
      return 22;
    case VFORMAT_VP9:
      return 23;
...
uint32_t DreamSysInfoFormat(vformat_t format, uint32_t decoderFormat)
{
  // Dream's vendor ABI expects VIDEO_DEC_FORMAT_*, not libamcodec's
  // VFORMAT_*. The values were traced from dreamvideosink for MPEG-2,
  // MPEG-4/H.263, H.264, HEVC, VP9, AVS, MJPEG and WMV. Dream's sink uses the
  // regular H.264 decoder ID for 4K/MVC as well.
  if (format == VFORMAT_H264 || format == VFORMAT_H264_4K2K || format == VFORMAT_H264MVC)
    return VIDEO_DEC_FORMAT_H264;

  return decoderFormat;
}

size_t DreamFindH264Nal(const uint8_t* data, size_t size, uint8_t type, size_t offset)
{
  for (size_t i = offset; i + 3 < size; ++i)
  {
    ...
    if (prefix != 0 && i + prefix < size && (data[i + prefix] & 0x1f) == type)
      return i;
  }
...
// AV1 is intentionally not part of the DreamOne/Two vendor decoder backend.
// Keep the imported parser available as reference without requiring libaom's
// private headers in the Kodi 22 build.
#if 0
...
bool CAMLCodec::OpenDecoder(CDVDStreamInfo &hints)
{
  ...
  // Kodi 22 removed CoreELEC's private advanced setting. Five seconds matches
  // the former decoder-stall default and still lets VideoPlayer recover.
  m_decoder_timeout = 5;
  m_buffer_level_ready = false;
#if defined(STB_AMLOGIC_NO_VDEC_INFO)
  // Dream's vendor 4.9 libamcodec does not return when GET_VDEC_INFO is not
  // implemented by the kernel.  It retries the ioctl forever, so capability
  // probing after the call is impossible.  Keep the optional query disabled
  // for these builds and use the demuxer aspect/frame-rate metadata instead.
  m_vdecInfoSupported.store(false, std::memory_order_relaxed);
#else
  m_vdecInfoSupported.store(true, std::memory_order_relaxed);
#endif
  ...
  // The Dreambox vendor decoder does not provide the CoreELEC Dolby Vision
  // integration. Keep HDR10/HLG active and explicitly use the normal path.
  am_private->gcodec.dv_enable = 0;
...
#if defined(STB_AMLOGIC_LEGACY_AMSTREAM)
  // DreamOne/Two use the same compressed-frame interface as E2's
  // dreamvideosink. libamcodec's CoreELEC frame-input ABI is not implemented
  // by the Dream vendor driver and would leave every codec_write() at EAGAIN.
  if (!OpenDreamVideo())
  {
  ...
  SetSpeed(m_speed);
#if defined(STB_AMLOGIC_LEGACY_AMSTREAM)
  SetPollDevice(m_amlVideoFile ? m_amlVideoFile->GetDescriptor() : -1);
#else
...
#if defined(STB_AMLOGIC_LEGACY_AMSTREAM)
bool CAMLCodec::OpenDreamVideo()
{
  m_dreamStreamType = DreamStreamType(am_private->video_format);
  if (m_dreamStreamType == -1)
  {
    CLog::Log(LOGERROR, "CAMLCodec::OpenDreamVideo - unsupported Dream video format {:d}",
              static_cast<int>(am_private->video_format));
    return false;
  }

  PosixFilePtr dreamVideoFile = std::make_shared<PosixFile>();
  if (!dreamVideoFile->Open("/dev/dvb/adapter0/video0", O_RDWR | O_NONBLOCK))
  {
    CLog::Log(LOGERROR,
              "CAMLCodec::OpenDreamVideo - cannot open /dev/dvb/adapter0/video0: {}",
              strerror(errno));
    return false;
  }

  // dreamvideosink opens this companion event device together with the DVB
  // video device. It reports decoder/output changes while the DVB fd provides
  // the actual compressed-frame backpressure via POLLOUT.
  PosixFilePtr dreamPollFile = std::make_shared<PosixFile>();
  if (!dreamPollFile->Open("/dev/amvideo_poll", O_RDWR | O_NONBLOCK))
  {
    CLog::Log(LOGERROR, "CAMLCodec::OpenDreamVideo - cannot open /dev/amvideo_poll: {}",
              strerror(errno));
    return false;
  }

  const int fd = dreamVideoFile->GetDescriptor();
  DreamVideoDecSysInfo sysInfo = {};
  sysInfo.format = DreamSysInfoFormat(am_private->video_format, am_private->gcodec.format);
  sysInfo.width = am_private->video_width;
  sysInfo.height = am_private->video_height;
  sysInfo.rate = am_private->video_rate;

  // Match dreamvideosink's ownership transition and start sequence exactly.
  // The Dream driver needs FREEZE before decoder configuration. CLEAR_BUFFER
  // at this point leaves its parser allocated but unable to consume frames.
  if (ioctl(fd, VIDEO_SELECT_SOURCE, VIDEO_SOURCE_MEMORY) < 0 ||
      ioctl(fd, VIDEO_FREEZE) < 0)
  {
    CLog::Log(LOGERROR, "CAMLCodec::OpenDreamVideo - DVB video setup failed: {}",
              strerror(errno));
    ioctl(fd, VIDEO_SELECT_SOURCE, VIDEO_SOURCE_DEMUX);
    return false;
  }

  if (ioctl(fd, DREAM_VIDEO_SET_DEC_SYSINFO, &sysInfo) < 0)
    CLog::Log(LOGWARNING,
              "CAMLCodec::OpenDreamVideo - VIDEO_SET_DEC_SYSINFO is unavailable: {}",
              strerror(errno));

  if (m_dreamStreamType != DREAM_STREAMTYPE_FROM_SYSINFO &&
      ioctl(fd, VIDEO_SET_STREAMTYPE, m_dreamStreamType) < 0)
  {
    CLog::Log(LOGERROR, "CAMLCodec::OpenDreamVideo - VIDEO_SET_STREAMTYPE failed: {}",
              strerror(errno));
    ioctl(fd, VIDEO_SELECT_SOURCE, VIDEO_SOURCE_DEMUX);
    return false;
  }

  // Route decoded frames through Kodi's amlvideo receiver. amvideo remains in
  // the chain for the vendor driver's display plane and is hidden until Kodi's
  // renderer has dequeued its first frame.
  SetVfmMap("default", "decoder ppmgr amlvideo deinterlace amvideo");

  if (ioctl(fd, VIDEO_PLAY) < 0 || ioctl(fd, VIDEO_SLOWMOTION, 0) < 0 ||
      ioctl(fd, VIDEO_FAST_FORWARD, 0) < 0 || ioctl(fd, VIDEO_CONTINUE) < 0)
  {
    CLog::Log(LOGERROR, "CAMLCodec::OpenDreamVideo - VIDEO_PLAY failed: {}", strerror(errno));
  ...
  m_dreamVideoFile = dreamVideoFile;
  m_dreamPollFile = dreamPollFile;
  m_dreamFirstWriteLogged = false;
  m_dreamBackpressureLogged = false;
  CLog::Log(LOGINFO,
            "CAMLCodec::OpenDreamVideo - native Dream frame backend, streamtype {:d}, "
            "codec format {:d}, sysinfo format {:d}, {:d}x{:d}, rate {:d}",
            m_dreamStreamType, static_cast<int>(am_private->video_format), sysInfo.format,
            am_private->video_width, am_private->video_height, am_private->video_rate);
  return true;
}

void CAMLCodec::CloseDreamVideo()
{
  if (!m_dreamVideoFile)
    return;

  const int fd = m_dreamVideoFile->GetDescriptor();
  ioctl(fd, VIDEO_CLEAR_BUFFER);
  ioctl(fd, VIDEO_STOP, 0);
  ioctl(fd, VIDEO_SELECT_SOURCE, VIDEO_SOURCE_DEMUX);
  m_dreamPollFile.reset();
  m_dreamVideoFile.reset();
  m_dreamStreamType = -1;
  SetVfmMap("default", m_defaultVfmMap);
}
#endif
...
void CAMLCodec::SaveDecoderState()
{
  const auto saveValue = [](const char* path, int& saved, bool& restore)
  {
    CSysfsPath sysfs{path};
    const auto value = sysfs.Get<int>();
    restore = value.has_value();
    if (restore)
      saved = value.value();
  };

  saveValue("/sys/module/amvdec_h264/parameters/dec_control", m_savedDecControl,
            m_restoreDecControl);
  saveValue("/sys/class/video/freerun_mode", m_savedFreerunMode, m_restoreFreerunMode);
  saveValue("/sys/module/amvdec_h265/parameters/nal_skip_policy", m_savedNalSkipPolicy,
            m_restoreNalSkipPolicy);
  saveValue("/sys/module/di/parameters/di_debug_flag", m_savedDiDebugFlag,
            m_restoreDiDebugFlag);
}

void CAMLCodec::RestoreDecoderState()
{
  if (m_restoreDecControl)
    CSysfsPath("/sys/module/amvdec_h264/parameters/dec_control", m_savedDecControl);
  if (m_restoreFreerunMode)
    CSysfsPath("/sys/class/video/freerun_mode", m_savedFreerunMode);
  if (m_restoreNalSkipPolicy)
    CSysfsPath("/sys/module/amvdec_h265/parameters/nal_skip_policy", m_savedNalSkipPolicy);
  if (m_restoreDiDebugFlag)
    CSysfsPath("/sys/module/di/parameters/di_debug_flag", m_savedDiDebugFlag);

  m_restoreDecControl = false;
  m_restoreFreerunMode = false;
  m_restoreNalSkipPolicy = false;
  m_restoreDiDebugFlag = false;
}
...
void CAMLCodec::Reset()
{
  ...
  if (!OpenDreamVideo())
  {
    CLog::Log(LOGERROR, "CAMLCodec::Reset - failed to restart native Dream decoder");
    m_opened = false;
    return;
  }
...
#if defined(STB_AMLOGIC_LEGACY_AMSTREAM)
bool CAMLCodec::AddDreamData(uint8_t* pData, size_t iSize, double dts, double pts)
{
  if (!m_opened || !m_dreamVideoFile || !m_dreamPollFile || !pData || iSize == 0)
    return false;

  am_packet_t* pkt = &am_private->am_pkt;
  pkt->data = pData;
  pkt->data_size = static_cast<int>(iSize);
  pkt->avpkt.data = pData;
  pkt->avpkt.size = static_cast<int>(iSize);
  pkt->newflag = 1;
  pkt->isvalid = 1;
  pkt->avduration = 0;
  ...
  {
    pkt->avpts = static_cast<uint64_t>(pts);
    m_state |= STATE_HASPTS;
  }

  pkt->avdts = dts == DVD_NOPTS_VALUE ? pkt->avpts : static_cast<uint64_t>(dts);
  if (m_cur_pts == DVD_NOPTS_VALUE && (m_state & STATE_HASPTS) == 0)
    pkt->avpts = pkt->avdts;

  if (set_header_info(am_private) < 0)
  {
    CLog::Log(LOGERROR, "CAMLCodec::AddDreamData - failed to prepare codec frame header");
    return false;
  }

  DreamVideoFrame frame = {};
  const double framePts = pts != DVD_NOPTS_VALUE ? pts : dts;
  if (framePts != DVD_NOPTS_VALUE && framePts > 0.0)
  {
    // Kodi timestamps use DVD_TIME_BASE (microseconds); Dream's DVB decoder
    // takes the same 90 kHz value generated by dreamvideosink.
    frame.pts = static_cast<uint64_t>(framePts * 9.0 / 100.0 + 0.5);
  }

  int segment = 0;
  const auto addSegment = [&frame, &segment](const uint8_t* data, size_t size)
  {
    if (data && size > 0 && segment < 8)
    {
      frame.bytes[segment] = static_cast<ssize_t>(size);
      frame.data[segment++] = data;
    }
  };

  if (pkt->hdr && pkt->hdr->data && pkt->hdr->size > 0)
    addSegment(reinterpret_cast<const uint8_t*>(pkt->hdr->data), pkt->hdr->size);

  bool reorderedH264Headers = false;
  static constexpr uint8_t dreamAnnexBZero = 0;
  if (am_private->video_format == VFORMAT_H264 ||
      am_private->video_format == VFORMAT_H264_4K2K ||
      am_private->video_format == VFORMAT_H264MVC)
  {
    // Kodi's AVC converter can produce the first AU as SEI,SPS,PPS,IDR. The
    // Dream parser does not start until SPS/PPS precede SEI, while the same AU
    // is accepted by generic Amlogic. Submit the existing NAL ranges in the
    // order used by dreamvideosink without copying or changing their size.
    const size_t packetSize = static_cast<size_t>(pkt->data_size);
    const size_t sei = DreamFindH264Nal(pkt->data, packetSize, 6, 0);
    const size_t sps = DreamFindH264Nal(pkt->data, packetSize, 7, 0);
    const size_t pps = DreamFindH264Nal(pkt->data, packetSize, 8, sps < packetSize ? sps : 0);
    const size_t idr = DreamFindH264Nal(pkt->data, packetSize, 5, pps < packetSize ? pps : 0);
    if (sei == 0 && sps > sei && pps > sps && idr > pps && idr < packetSize)
    {
      addSegment(pkt->data + sps, idr - sps);
      addSegment(pkt->data, sps);
      // Dream's H.264 parser requires the IDR start code to be four bytes.
      // FFmpeg may emit this particular boundary as 00 00 01. The proprietary
      // h264parse/dreamvideosink path inserts one leading zero as well.
      if (idr + 2 < packetSize && pkt->data[idr] == 0 && pkt->data[idr + 1] == 0 &&
          pkt->data[idr + 2] == 1)
        addSegment(&dreamAnnexBZero, 1);
      addSegment(pkt->data + idr, packetSize - idr);
      reorderedH264Headers = true;
      if (!m_dreamFirstWriteLogged)
        CLog::Log(LOGINFO,
                  "CAMLCodec::AddDreamData - normalized Dream H.264 AU to SPS/PPS,SEI,IDR");
    }
  }

  if (!reorderedH264Headers)
    addSegment(pkt->data, pkt->data_size);
  ...
  for (int i = 0; i < segment; ++i)
    remaining += frame.bytes[i];

  const ssize_t frameBytes = remaining;
  bool wroteAny = false;
  while (remaining > 0)
  {
    pollfd fds[2] = {};
    fds[0].fd = m_dreamVideoFile->GetDescriptor();
    fds[0].events = POLLOUT | POLLPRI;
    fds[1].fd = m_dreamPollFile->GetDescriptor();
    fds[1].events = POLLIN;

    const int pollResult = poll(fds, 2, wroteAny ? -1 : 0);
    if (pollResult < 0)
    {
      if (errno == EINTR)
        continue;
      CLog::Log(LOGERROR, "CAMLCodec::AddDreamData - Dream video poll failed: {}",
                strerror(errno));
      return false;
    }
    ...
    {
      CLog::Log(LOGERROR, "CAMLCodec::AddDreamData - Dream video poll error 0x{:x}",
                static_cast<unsigned int>(fds[0].revents));
      return false;
    }

    if (pollResult == 0 || (fds[0].revents & POLLOUT) == 0)
    {
      if (!m_dreamBackpressureLogged)
      {
        CLog::Log(LOGINFO,
                  "CAMLCodec::AddDreamData - waiting for native Dream decoder input");
        m_dreamBackpressureLogged = true;
      }
      return false;
    }

    const int written = ioctl(fds[0].fd, DREAM_VIDEO_SET_FRAME, &frame);
    if (written < 0)
    {
      if (errno == EINTR)
        continue;
      if (errno == EAGAIN || errno == EWOULDBLOCK)
      {
        ...
        {
          CLog::Log(LOGINFO,
                    "CAMLCodec::AddDreamData - native Dream decoder reported backpressure");
          m_dreamBackpressureLogged = true;
        }
        if (!wroteAny)
          return false;
      ...
      CLog::Log(LOGERROR, "CAMLCodec::AddDreamData - VIDEO_SET_FRAME failed: {}",
                strerror(errno));
      return false;
    }

    // VIDEO_SET_FRAME returns the number of bytes accepted, not a normal
    // zero-on-success ioctl result. dreamvideosink advances the segment array
    // and submits it again until the complete access unit has been consumed.
    if (written == 0)
    {
      if (!m_dreamBackpressureLogged)
      {
        CLog::Log(LOGINFO,
                  "CAMLCodec::AddDreamData - native Dream decoder accepted zero bytes");
        m_dreamBackpressureLogged = true;
      }
      if (!wroteAny)
        return false;
    ...
    {
      CLog::Log(LOGERROR,
                "CAMLCodec::AddDreamData - invalid VIDEO_SET_FRAME result {:d}/{:d}", written,
                static_cast<int>(remaining));
      return false;
    }

    ssize_t advance = written;
    for (int i = 0; i < segment && advance > 0; ++i)
    {
      if (frame.bytes[i] <= 0)
        continue;
      if (advance >= frame.bytes[i])
      {
        advance -= frame.bytes[i];
        frame.bytes[i] = 0;
        frame.data[i] = nullptr;
      }
      else
      {
        frame.data[i] += advance;
        frame.bytes[i] -= advance;
        advance = 0;
      }
    }
  ...
  dumpfile_write(am_private, pkt->data, pkt->data_size);
  pkt->isvalid = 0;
  pkt->data_size = 0;
  ...
  {
    CLog::Log(LOGINFO,
              "CAMLCodec::AddDreamData - first native Dream frame submitted, {:d} segments, "
              "{:d} bytes",
              segment, static_cast<int>(frameBytes));
    m_dreamFirstWriteLogged = true;
  }

  CLog::Log(LOGDEBUG, LOGVIDEO,
            "CAMLCodec::AddDreamData - submitted {:d} segments, {:d} bytes, pts {:.3f}",
            segment, static_cast<int>(iSize), pts / DVD_TIME_BASE);
  return true;
}
#endif
...
{
#if defined(STB_AMLOGIC_LEGACY_AMSTREAM)
  return AddDreamData(pData, iSize, dts, pts);
#else
...
#if defined(STB_AMLOGIC_LEGACY_AMSTREAM)
  codec_poll_fd[0].events = POLLIN | POLLPRI;
#else
...
float CAMLCodec::GetBufferLevel(int new_chunk, int &data_len, int &free_len)
{
  ...
  int status;
#if defined(STB_AMLOGIC_LEGACY_AMSTREAM)
  // VIDEO_SET_FRAME is atomic and reports its own EAGAIN backpressure. There
  // is no parser-ring fill level for this native Dream frame interface.
  data_len = 0;
  free_len = 4 * 1024 * 1024;
  return 50.0f;
#else
  status = m_dll->codec_get_vbuf_state(&am_private->vcodec, &bs);
#endif
  ...
  if (status != 0)
  {
    data_len = 0;
    free_len = 0;
    if (!m_vbufStateErrorLogged)
    {
      CLog::Log(LOGERROR, "CAMLCodec::GetBufferLevel - video buffer status failed: {}",
                strerror(errno));
      m_vbufStateErrorLogged = true;
    }
    return 100.0f;
  }
...
CDVDVideoCodec::VCReturn CAMLCodec::GetPicture(VideoPicture *pVideoPicture)
{
...
#if defined(STB_AMLOGIC_LEGACY_AMSTREAM)
  // VIDEO_SET_FRAME is an access-unit interface, not libamcodec's parser ring.
  // Always probe amlvideo once input has been accepted.  Until a decoded frame
  // is ready, VC_BUFFER tells VideoPlayer to submit more access units.  Treating
  // the synthetic buffer level as a filled parser ring returns VC_NONE here and
  // deadlocks after the first packet (spinner forever, decoder frame count 0).
  const bool shouldDequeue = !m_drain && m_buffer_level_ready;
#else
  const bool shouldDequeue = !m_drain && m_buffer_level_ready &&
                             buffer_level > m_minimum_buffer_level;
#endif

  if (shouldDequeue && (ret = DequeueBuffer()) == 0)
  {
    ...
    if (m_vdecInfoSupported.load(std::memory_order_relaxed))
    {
      ...
      else if (m_vdecInfoSupported.exchange(false, std::memory_order_relaxed))
      {
        CLog::Log(LOGINFO,
                  "CAMLCodec: optional vdec-info ioctl is unavailable; using stream aspect and frame rate");
      }
    }
  ...
  else if (ret != EAGAIN || elapsed_since_last_frame > std::chrono::seconds(m_decoder_timeout))
  {
    ...
    m_tp_last_frame = std::chrono::system_clock::now();
#if defined(STB_AMLOGIC_LEGACY_AMSTREAM)
    // Repeated codec_reset() calls race Dream's proprietary VFM teardown and
    // eventually crash in libamcodec's codec_init().  Fail this decoder
    // instance cleanly; VideoPlayer can stop or select another decoder.
    return CDVDVideoCodec::VC_ERROR;
#else
...
void CAMLCodec::SetSpeed(int speed)
{
  ...
  {
    if (ioctl(fd, VIDEO_FREEZE) < 0)
      CLog::Log(LOGWARNING, "CAMLCodec::SetSpeed - VIDEO_FREEZE failed: {}", strerror(errno));
  }
  else
  {
    if (ioctl(fd, VIDEO_CONTINUE) < 0)
      CLog::Log(LOGWARNING, "CAMLCodec::SetSpeed - VIDEO_CONTINUE failed: {}", strerror(errno));
...
void CAMLCodec::SetVideoRect(const CRect &SrcRect, const CRect &DestRect)
{
  ...
  RenderStereoMode guiStereoMode = CServiceBroker::GetWinSystem()->GetGfxContext().GetStereoMode();
  ...
  CLog::Log(LOGDEBUG, "CAMLCodec::SetVideoRect:m_guiStereoMode({:d})", static_cast<int>(m_guiStereoMode));
  CLog::Log(LOGDEBUG, "CAMLCodec::SetVideoRect:m_guiStereoView({:d})", static_cast<int>(m_guiStereoView));
#endif
...
unsigned int CAMLCodec::GetDecoderVideoRate()
{
  if (m_speed != DVD_PLAYSPEED_NORMAL || m_pollDevice < 0 ||
      !m_vdecInfoSupported.load(std::memory_order_relaxed))
    return 0;
  ...
  if (m_vdecInfoSupported.exchange(false, std::memory_order_relaxed))
  {
    CLog::Log(LOGINFO,
              "CAMLCodec: optional vdec-info ioctl is unavailable; using stream aspect and frame rate");
  }
