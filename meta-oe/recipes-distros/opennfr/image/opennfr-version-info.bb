...
do_install() {
    ...
    echo "version=${DISTRO_VERSION}" >> ${D}${sysconfdir}/image-version
    echo "build=${BUILD_VERSION}" >> ${D}${sysconfdir}/image-version
    echo "date=${DATETIME}" >> ${D}${sysconfdir}/image-version
    echo "comment=openNFR" >> ${D}${sysconfdir}/image-version
    echo "target=9" >> ${D}${sysconfdir}/image-version
    echo "creator=openNFR" >> ${D}${sysconfdir}/image-version
