...
/* -------------------------------------------------------------------------- */
static void aes_addRoundKey_iv_encA(uint8_t *buf, uint8_t *key)
{
	register uint8_t i = 16;
...
/* -------------------------------------------------------------------------- */
static void aes_addRoundKey_iv_encB(uint8_t *buf, uint8_t *key)
{
	register uint8_t i = 16;
...
/* -------------------------------------------------------------------------- */
static void aes_addRoundKey_iv_decA(uint8_t *buf, uint8_t *key)
{
	register uint8_t i = 16;
	while (i--) key[i] = buf[i];
} /* aes_addRoundKey_iv */
...
/* -------------------------------------------------------------------------- */
static void aes_addRoundKey_iv_decB(uint8_t *buf, uint8_t *curr_key, uint8_t *next_key)
{
	register uint8_t i = 16;
	while (i--) buf[i] ^= curr_key[i], curr_key[i] = next_key[i];
} /* aes_addRoundKey_iv */
...
/* -------------------------------------------------------------------------- */
void aes256_init_ex(aes256_context *ctx, uint8_t *k, uint8_t *iv)
{
	uint8_t rcon = 1;
	register uint8_t i;

	for (i = 0; i < sizeof(ctx->key); i++) ctx->enckey[i] = ctx->deckey[i] = k[i];
	for (i = 0; i < sizeof(ctx->curr_iv); i++) ctx->curr_iv[i] = iv[i];
	for (i = 8; --i;) aes_expandEncKey(ctx->deckey, &rcon);
} /* aes256_init */
...
void aes256_encrypt_cbc(aes256_context *ctx, uint8_t *buf)
{
	...
	aes_addRoundKey_iv_encA( buf, ctx->curr_iv);
	aes_addRoundKey_cpy(buf, ctx->enckey, ctx->key);
	for(i = 1, rcon = 1; i < 14; ++i)
	{
		aes_subBytes(buf);
	...
	aes_expandEncKey(ctx->key, &rcon);
	aes_addRoundKey(buf, ctx->key);
	aes_addRoundKey_iv_encB( buf, ctx->curr_iv);
} /* aes256_encrypt */
...
void aes256_decrypt_cbc(aes256_context *ctx, uint8_t *buf)
{
	...
	aes_addRoundKey_iv_decA( buf, ctx->next_iv);
	aes_addRoundKey_cpy(buf, ctx->deckey, ctx->key);
	aes_shiftRows_inv(buf);
	aes_subBytes_inv(buf);
	...
	aes_addRoundKey( buf, ctx->key);
	aes_addRoundKey_iv_decB( buf, ctx->curr_iv, ctx->next_iv);
} /* aes256_decrypt */
