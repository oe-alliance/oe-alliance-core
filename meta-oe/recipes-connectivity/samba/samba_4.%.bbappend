...
do_install_prepend() {
    ...
    # these files needs to be installed, because of sed in meta-OE bb
    install -d ${D}${bindir}
    install -d ${D}${sbindir}
    cp -f ${S}/source4/scripting/bin/samba-gpupdate ${D}${sbindir}
    cp -f ${S}/source4/scripting/bin/samba_upgradedns ${D}${sbindir}
    cp -f ${S}/source4/scripting/bin/samba_spnupdate ${D}${sbindir}
    cp -f ${S}/source4/scripting/bin/samba_kcc ${D}${sbindir}
    cp -f ${S}/source4/scripting/bin/samba_dnsupdate ${D}${sbindir}
    cp -f ${S}/source4/scripting/bin/samba-tool ${D}${bindir}
