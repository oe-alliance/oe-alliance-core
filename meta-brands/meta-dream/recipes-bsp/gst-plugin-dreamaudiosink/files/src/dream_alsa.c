                          ...
                          int passthrough)
{
    ...
    /* No silence-pad sw_params here. With dmix in the chain
     * (dreamhdmi → softvol → hdmidmix → hw:0,0) the silence-pad logic
     * accidentally outraced our writes: the slave drained faster than
     * the decoder produced PCM, dmix saw the gap as underrun, filled
     * with zeros, and the next writes were dropped because the buffer
     * cursor had already moved past. test5.mkv played for ~1 s of
     * initial buffer, then silence forever. Default sw_params (XRUN on
     * underrun + writei recovery via snd_pcm_recover) match how
     * GstBaseSink expects an ALSA sink to behave. */
