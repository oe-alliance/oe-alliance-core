...
IMAGE_CMD:hdfastboot8gb () {
    ...
    echo "bootargs=androidboot.hardware=bigfish androidboot.serialno=0123456789 androidboot.selinux=disabled hbcomp=/dev/block/mmcblk0p13 root=/dev/mmcblk0p13 androidboot.dtbo_idx=0" >> ${WORKDIR}/STARTUP_ANDROID
    ...
    echo "bootargs=androidboot.hardware=bigfish androidboot.serialno=0123456789 androidboot.selinux=disabled hbcomp=/dev/block/mmcblk0p13 root=/dev/mmcblk0p13 androidboot.dtbo_idx=0" >> ${WORKDIR}/STARTUP_ANDROID_DISABLE_LINUXSE
    ...
    echo -n boot-recovery > ${WORKDIR}/misc-boot.img
