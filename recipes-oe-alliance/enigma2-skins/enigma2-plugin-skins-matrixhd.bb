...
do_install() {
	install -d ${D}/usr/
	cp -rp ${S}/usr/* ${D}/usr/
	cp -p ${S}/usr/lib/enigma2/python/Renderer/RunningText.py ${D}/temp/RunningText.py
	chmod -R a+rX ${D}/usr/
}

pkg_preinst_matrixhd-components() {
#!/bin/sh
if [ ! -f /usr/lib/enigma2/python/Renderer/RunningText.pyo ];then
	cp -rp /temp/RunningText.py /usr/lib/enigma2/python/Renderer/RunningText.py
fi
}
