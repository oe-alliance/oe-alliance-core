...
# File: recipes_azbox_azbox-enigma2-packages_VideoSettingsSetup_plugin.pyo (Python 2.6)
...
class videoSettingsSetup(Screen, ConfigListScreen):
    ...
    def keyBlueConfirm(self, confirmed):
        if not confirmed:
            print 'not confirmed'
        elif self.contrastEntry is not None:
...
def startSetup(menuid):
    if menuid != 'system':
        return []
    return [
        (_('Video settings'), videoSettingsSetupMain, 'videoSettings_setup', 41)]
